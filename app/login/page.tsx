...
export default function LoginPage() {
  ...
  // Reset links land on /auth/callback (which exchanges the code for a
  // session) and are then forwarded to the set-new-password page.
  ...
  const showForgot = (forgot: boolean) => {
    setIsForgot(forgot);
    setError(null);
    setMessage(null);
    setPassword("");
          ...
          >
            {isForgot && (
              <p className="text-sm text-charcoal/50 -mt-1">
                Enter the email for your account and we&apos;ll send you a link
            ...
            {!isForgot && (
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="block text-xs font-semibold text-charcoal/60 uppercase tracking-wider">
                    Password
                  </label>
                  {!isRegister && (
                    <button
